...
if __name__ == '__main__':
    ...
    # 1. Temporarily silence system-level stderr
    stderr_fd = sys.stderr.fileno()
    copied_fd = os.dup(stderr_fd)
    devnull = os.open(os.devnull, os.O_WRONLY)
    os.dup2(devnull, stderr_fd)

    try:
    # 2. Import the fclist package while stderr is muted
        fontinfo = fcmatch(wanted)
    finally:
        # 3. Restore stderr so you can see your own errors/logs
        os.dup2(copied_fd, stderr_fd)
        os.close(devnull)
        os.close(copied_fd)
